...
export const adminApi = {
  ...
  // POST /api/v1/admin/transactions/trigger-scraping
  triggerScraping: async (): Promise<{ success: boolean; message: string }> => {
    try {
      const response = await fetch(`${API_BASE_URL}/admin/transactions/trigger-scraping`, {
        method: 'POST',
        headers: getHeaders(),
      });
      return await response.json();
      ...
      throw new Error('Gagal memicu pengecekan mutasi');
    }
  },
  ...
  flashSales: {
    ...
    create: async (data: any): Promise<ApiResponse<any>> => {
      try {
        ...
        console.error('Error creating rebutan kartcis:', error);
        throw error;
      }
    },
    ...
    update: async (id: number | string, data: any): Promise<ApiResponse<any>> => {
      try {
        ...
        console.error('Error updating rebutan kartcis:', error);
        throw error;
      }
    },
    ...
    delete: async (id: number | string): Promise<ApiResponse<any>> => {
      try {
        ...
        console.error('Error deleting rebutan kartcis:', error);
        throw error;
      }
    },
  },
