...
export function EventCard({ event, onClick }: EventCardProps) {
  ...
  const totalAvailable = event.ticketTypes && event.ticketTypes.length > 0 
    ...
    : 0;
  const soldOutPercentage = event.quota > 0 
    ? ((event.quota - totalAvailable) / event.quota) * 100
    : 0;
  
  // Check if any ticket has discount
  const hasDiscount = event.ticketTypes && event.ticketTypes.some(t => t.originalPrice && t.originalPrice > t.price);
  const lowestTicket = event.ticketTypes && event.ticketTypes.length > 0 
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer group bg-white border-gray-200" onClick={onClick}>
      <div className="relative aspect-[16/9] overflow-hidden">
        ...
        {event.status === 'sold-out' && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
            <Badge className="bg-red-600 border-0 text-lg px-4 py-2">
              SOLD OUT
            </Badge>
          </div>
        )}
        {event.status === 'cancelled' && (
          <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
            <Badge className="bg-gray-900 border-0 text-lg px-4 py-2">
              DIBATALKAN
            </Badge>
          </div>
        )}
        
        {event.isFeatured && !event.status && (
          <Badge className="absolute top-3 left-3 bg-orange-500 border-0 text-sm">
            Populer
          </Badge>
        )}
        <Badge className="absolute top-3 right-3 bg-white text-gray-900 border-0 text-sm">
          {event.category}
        </Badge>
      </div>
      ...
      <div className="p-4">
        ...
        {soldOutPercentage > 80 && event.status !== 'sold-out' && event.status !== 'cancelled' && (
          <div className="mb-3">
